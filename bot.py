...
def main_keyboard(show_trial=False):
    ...
    buttons.append([InlineKeyboardButton("🛒 اشترك الآن", callback_data="subscribe")])
    ...
    if data == "subscribe":
        ...
        if invite_url:
            msg = (
                ...
                f"• المدة: 48 ساعة\n"
                f"• تنتهي في: {expires.strftime('%Y/%m/%d %H:%M')}\n\n"
                f"🔗 *رابط دخولك الخاص:*\n{invite_url}\n\n"
                f"⚠️ هذا الرابط خاص بك فقط.\n"
                f"بعد انتهاء التجربة سيتم إزالتك تلقائياً.\n"
                f"اشترك قبل الانتهاء للاستمرار 👇"
            )
        else:
            msg = (
                f"🎁 *تم تفعيل فترة التجربة المجانية!*\n\n"
        ...
        if admin_id:
            ...
            await context.bot.send_message(
                ...
                text=(
                    ...
                    f"📱 {phone}\n"
                    f"🆔 `{uid}`\n"
                    f"⏰ تنتهي: {expires.strftime('%Y/%m/%d %H:%M')}"
                ),
                parse_mode="Markdown"
            )
        ...
        await query.edit_message_text(
            ...
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="subscribe")]])
        )
        ...
        try:
            ...
            invite_url = link.invite_link
            logger.info(f"Invite link for {uid}: {invite_url}")
        except Exception as e:
...
async def delete_system_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Delete system messages like 'joined via invite link', 'left the group' etc."""
    try:
...
def main():
    ...
    # Delete system messages in the group
