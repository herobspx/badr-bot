...
def main_keyboard():
    return InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton("🔗 تجديد رابط الدخول", callback_data="renew_link")],
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_back":
        ...
        if not sub:
            await query.answer("❌ لا يوجد اشتراك فعال", show_alert=True)
            return

        exp = datetime.fromisoformat(sub["expires_at"])

        if exp <= datetime.now():
            await query.answer("❌ اشتراكك منتهي", show_alert=True)
            return
        ...
        try:
            ...
            await query.message.reply_text(link.invite_link)
            await query.answer("✅ تم إرسال رابط جديد", show_alert=False)

        except Exception as e:

            logger.error(f"Renew link error: {e}")

            await query.answer(
                "⚠️ فشل إنشاء الرابط، تأكد أن البوت أدمن بالقناة",
                show_alert=True
            )
        ...
        saved = await save_subscriber(target_uid, plan_key, expires)

        # لا نرسل رابط الدخول إلا بعد التأكد أن الاشتراك انحفظ في قاعدة البيانات.
        confirmed_sub = await get_subscriber(target_uid)
        if not saved or not confirmed_sub:
            await query.edit_message_text(
                "⚠️ لم يتم حفظ الاشتراك في قاعدة البيانات.\n"
                "تأكد من جداول Supabase ووجود unique/primary key على عمود uid."
            )
            return
        ...
        try:
            ...
            await query.edit_message_text("✅ تم قبول الاشتراك وحفظه وإرسال رابط الدخول لمدة 5 دقائق.")
        except Exception as e:
            logger.error(f"Invite link error: {e}")
            await query.edit_message_text("✅ تم حفظ الاشتراك لكن فشل توليد الرابط. تأكد أن البوت أدمن في القناة الخاصة.")
