...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid == str(admin_id):
        await update.message.reply_text("👑 <b>لوحة تحكم الأدمن</b>\n\nاختر من القائمة:", parse_mode="HTML", reply_markup=admin_keyboard())
        return

    if await is_verified(uid):
        show_trial = not await used_trial(uid) and not await get_subscriber(uid)
        await update.message.reply_text(
            f"أهلاً {user.first_name}! 👋\n\nمرحباً بك في قناة <b>عقود الأوبشن</b> 📈\n\nاشترك الآن للحصول على:\n• ✅ إشارات يومية احترافية\n• 📊 تحليلات دقيقة\n• 🔔 تنبيهات فورية\n\nاختر من القائمة:",
            parse_mode="HTML", reply_markup=main_keyboard(show_trial=show_trial)
        )
    else:
        await update.message.reply_text(
...
async def receive_contact(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ <b>تم التحقق بنجاح!</b>\n\nأهلاً بك 🎉", parse_mode="HTML", reply_markup=ReplyKeyboardRemove())
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin and not await is_verified(uid) and not data.startswith(("approve_","reject_")) and data != "channel_verify":
    ...
    if data == "admin_verifypost":
        ...
        await query.edit_message_text(f"👥 <b>المشتركون المحفوظون:</b> {count} شخص", parse_mode="HTML", reply_markup=admin_keyboard())
        ...
        for row in expired_list:
            try:
                ...
                await context.bot.send_message(chat_id=int(row["uid"]), text=msg, parse_mode="HTML", reply_markup=main_keyboard())
                removed += 1
        ...
        await query.edit_message_text("📦 <b>خطط الاشتراك</b>\n\nاختر الخطة المناسبة:", parse_mode="HTML", reply_markup=plans_keyboard())
        ...
        await query.edit_message_text(f"🎁 <b>تم تفعيل فترة التجربة المجانية!</b>\n\n• المدة: 48 ساعة\n• تنتهي في: {expires_str}\n\n⏳ سيتم إضافتك للقناة خلال دقائق.", parse_mode="HTML")
        try:
            ...
            await context.bot.send_message(chat_id=int(uid), text=f"🔗 <b>رابط دخولك للقناة:</b>\n{link.invite_link}\n\n⚠️ الرابط صالح لمدة <b>5 دقائق</b> فقط.\nادخل القناة الآن! ⏰", parse_mode="HTML")
        except Exception as e:
            logger.error(f"Trial invite error: {e}")
        if admin_id:
            phone = await get_verified_phone(uid)
            await context.bot.send_message(chat_id=admin_id, text=f"🎁 <b>فترة تجربة جديدة</b>\n\n👤 {user.full_name}\n📱 {phone}\n🆔 <code>{uid}</code>\n⏰ تنتهي: {expires_str}", parse_mode="HTML")
        ...
        await query.edit_message_text(
            f"✅ اخترت خطة *{plan['label']}* بـ {plan['price']} ريال\n\n💳 <b>بيانات التحويل البنكي:</b>\n• البنك: {BANK_NAME}\n• الاسم: {ACCOUNT_NAME}\n• الآيبان: <code>{IBAN}</code>\n\n📌 <b>بعد التحويل:</b>\nأرسل صورة إيصال التحويل هنا وسيتم مراجعتها خلال دقائق.\n\n⚠️ تأكد أن اسمك ظاهر في الإيصال.",
            parse_mode="HTML", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="back_main")]])
        )

    elif data == "back_main":
        show_trial = not await used_trial(uid) and not await get_subscriber(uid)
        await query.edit_message_text("مرحباً بك في قناة <b>عقود الأوبشن</b> 📈\n\nاختر من القائمة:", parse_mode="HTML", reply_markup=main_keyboard(show_trial=show_trial))
        ...
        msg = f"🎉 <b>تم تأكيد اشتراكك!</b>\n\n• الخطة: {plan['label']}\n• ينتهي في: {expires.strftime('%Y/%m/%d')}\n\n🔗 <b>رابط دخولك الخاص:</b>\n{invite_url}\n\n⚠️ هذا الرابط خاص بك فقط ويُستخدم مرة واحدة." if invite_url else f"🎉 <b>تم تأكيد اشتراكك!</b>\n\n• الخطة: {plan['label']}\n• ينتهي في: {expires.strftime('%Y/%m/%d')}\n\n⏳ سيتم إضافتك للقناة خلال دقائق."
        admin_note = f"✅ تم قبول <code>{t_uid}</code> — {plan['label']}\n🔗 تم إرسال الرابط" if invite_url else f"✅ تم قبول <code>{t_uid}</code> — {plan['label']}\n⚠️ فشل توليد الرابط"
        await context.bot.send_message(chat_id=int(t_uid), text=msg, parse_mode="HTML")
        await query.edit_message_text(admin_note, parse_mode="HTML")
        ...
        await delete_pending(t_uid)
        await context.bot.send_message(chat_id=int(t_uid), text="❌ <b>تم رفض طلبك.</b>\n\nتأكد من صحة الإيصال وأعد الإرسال أو تواصل معنا.", parse_mode="HTML")
        await query.edit_message_text(f"❌ تم رفض طلب <code>{t_uid}</code>", parse_mode="HTML")
...
async def receive_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.photo:
        await context.bot.send_photo(chat_id=admin_id, photo=update.message.photo[-1].file_id, caption=caption, parse_mode="HTML", reply_markup=kb)
    elif update.message.document:
        await context.bot.send_document(chat_id=admin_id, document=update.message.document.file_id, caption=caption, parse_mode="HTML", reply_markup=kb)
    else:
        await update.message.reply_text("📎 أرسل صورة الإيصال من فضلك.")
        return
    await update_pending_status(uid, "receipt_sent")
    await update.message.reply_text("✅ <b>تم استلام إيصالك!</b>\n\nسيتم مراجعته وتفعيل اشتراكك خلال دقائق. 🙏", parse_mode="HTML")
